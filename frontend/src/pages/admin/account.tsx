...
const InnerLayout = () => {
  ...
  if (failed) {
    return <div role="alert">无法读取账户管理方式，请刷新重试。</div>;
  }
  if (loading || !options) {
    return <Loading />;
  }
  ...
  if (options.enabled) {
    return <Flex direction="column" gap="3" className="p-4">
      ...
      <p>当前账户：{account?.username}</p>
      <Badge color="blue">统一账户 · Komari 探针管理</Badge>
      <p>用户名、密码和双因素认证由 ASWired 统一管理，请联系 ASWired 管理员修改。</p>
      <p>此账户仅可进入探针管理页面。</p>
      <div><Button onClick={() => window.location.assign("/api/logout")}>退出登录</Button></div>
    </Flex>;
  }
