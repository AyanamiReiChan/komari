...
export default function Sessions() {
  ...
  if (failed) return <div role="alert">无法读取会话管理方式，请刷新重试。</div>;
  if (!options) return <Loading />;
  if (!options.enabled) return <StandaloneSessions />;
  return <Flex direction="column" gap="3" className="p-4">
    <h1 className="text-2xl font-bold">登录会话</h1>
    <p>当前使用统一账户登录。管理员停用账户或重置密码后，已有会话将失效。</p>
    <div><Button onClick={() => window.location.assign("/api/logout")}>退出当前会话</Button></div>
  </Flex>;
}

function StandaloneSessions() {
